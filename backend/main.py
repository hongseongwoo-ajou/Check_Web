...
def fetch_chess_ratings(chess_username: str) -> dict:
    """
    Chess.com PubAPI에서 blitz/rapid 레이팅 조회.
    ...
    try:
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            return {
                "rating_blitz": data.get("chess_blitz", {}).get("last", {}).get("rating"),
...
def update_user_rating(user_id: int, chess_username: str):
    ...
    with get_db() as conn:
        conn.execute(
            """UPDATE users
               SET rating_blitz = ?, rating_rapid = ?, rating_updated_at = ?
               WHERE id = ?""",
            (
...
async def refresh_chess_rating(user=Depends(get_current_user)):
    ...
    with get_db() as conn:
        conn.execute(
            ...
            (rating_blitz, rating_rapid, now, user["id"]),
        )
        conn.commit()
...
async def get_group_members(group_id: int, user=Depends(get_current_user)):
    with get_db() as conn:
        ...
        members = conn.execute(
            """SELECT u.id, u.nickname, u.chess_username, u.rating_blitz, u.rating_rapid,
                      u.rating_updated_at, ug.role
               FROM users u
               JOIN user_groups ug ON u.id = ug.user_id
               WHERE ug.group_id = ?
               ORDER BY COALESCE(u.rating_blitz, 0) DESC""",
            (group_id,),
        ).fetchall()

    member_list = [dict(m) for m in members]
    linked = sum(1 for m in member_list if m["chess_username"])
    rated  = [m["rating_blitz"] for m in member_list if m["rating_blitz"]]
    avg_blitz = round(sum(rated) / len(rated)) if rated else None
...
async def create_poll(group_id: int, req: CreatePollRequest, user=Depends(get_current_user)):
    with get_db() as conn:
        ...
        existing = conn.execute(
            "SELECT id FROM polls WHERE group_id = ? AND status IN ('voting','playing')",
            (group_id,),
        ).fetchone()
        if existing:
...
async def get_active_poll(group_id: int, user=Depends(get_current_user)):
    with get_db() as conn:
        if not conn.execute(
            "SELECT 1 FROM user_groups WHERE user_id = ? AND group_id = ?",
            (user["id"], group_id),
        ...
        poll = conn.execute(
            "SELECT * FROM polls WHERE group_id = ? AND status IN ('voting','playing') ORDER BY id DESC LIMIT 1",
            (group_id,),
        ).fetchone()

        if not poll:
            return {"poll": None, "votes": [], "my_vote": False, "matches": []}
...
async def close_poll(poll_id: int, user=Depends(get_current_user)):
    with get_db() as conn:
        ...
        voters = conn.execute(
            """SELECT u.id, u.nickname, u.rating_blitz, ug.role
               FROM poll_votes pv
               JOIN users u ON pv.user_id = u.id
        ...
        pairs, bye_player = make_pairs(voter_list, conn, poll["group_id"])
...
async def get_leaderboard(group_id: int, user=Depends(get_current_user)):
    with get_db() as conn:
        ...
        members = conn.execute(
            """SELECT u.id, u.nickname, u.chess_username, u.rating_blitz
               FROM users u
               JOIN user_groups ug ON u.id = ug.user_id
               WHERE ug.group_id = ?""",
            (group_id,),
    ...
    return {
        ...
        "current_role": current_role,
        "settings": {"pts_win": pts_win, "pts_draw": pts_draw, "pts_loss": pts_loss},
        "standings": standings,
    }
...
async def update_group_settings(group_id: int, req: GroupSettingsRequest, user=Depends(get_current_user)):
    with get_db() as conn:
        _check_admin(conn, user["id"], group_id)
        conn.execute(
            """INSERT INTO group_settings (group_id, pts_win, pts_draw, pts_loss)
               VALUES (?, ?, ?, ?)
               ON CONFLICT(group_id) DO UPDATE SET
                   pts_win = excluded.pts_win,
                   pts_draw = excluded.pts_draw,
                   pts_loss = excluded.pts_loss""",
            (group_id, req.pts_win, req.pts_draw, req.pts_loss),
        )
        conn.commit()
    return {"pts_win": req.pts_win, "pts_draw": req.pts_draw, "pts_loss": req.pts_loss}
...
async def record_match_result(match_id: int, req: MatchResultRequest, user=Depends(get_current_user)):
    with get_db() as conn:
        ...
        if match["status"] == "finished":
            raise HTTPException(status_code=400, detail="이미 결과가 기록된 매치입니다.")
...
async def get_dashboard(group_id: int, user=Depends(get_current_user)):
    with get_db() as conn:
        ...
        # 블리츠 레이팅 내림차순 정렬 (미연동 멤버는 하단)
        members = conn.execute(
            """SELECT u.id, u.nickname, u.chess_username,
                      u.rating_blitz, u.rating_rapid, u.rating_updated_at,
                      ug.joined_at
               FROM users u
               JOIN user_groups ug ON u.id = ug.user_id
               WHERE ug.group_id = ?
